...
const ThankYou = ({ anonymousKey }: { anonymousKey: string }) => {
  console.log("ThankYou component rendered with anonymousKey:", anonymousKey); // Add this line
  ...
  const {
    ...
    queryKey: ["thank-you"],
    queryFn: async () => {
      console.log("queryFn started for anonymousKey:", anonymousKey); // Add this line
      try {
        ...
        console.error("Error fetching order data:", error); // Add this line
        throw error;
      }
    },
  ...
  if (!res?.result || error) {
    console.error("Render error or no result:", error); // Add this line
    return (
